...
interface Company { id: string; name: string; ruc: string; dv: string; itbms_rate: number; address: string; phone: string; email: string; timezone: string; logo_url: string | null }
...
function CompanySettings() {
  ...
  async function fetchCompany() {
    try {
      ...
      if (data) { setCompany(data); setFormData({ name: data.name || '', ruc: data.ruc || '', dv: data.dv || '', itbms_rate: data.itbms_rate || 0.07, address: data.address || '', phone: data.phone || '', email: data.email || '', timezone: data.timezone || 'America/Panama', logo_url: data.logo_url || '' }) }
  ...
  async function handleLogoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const fileName = `company-${company.id}-logo.${fileExt}`
      ...
      alert('Error uploading logo. Make sure the "images" storage bucket exists.')
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 max-w-2xl">
      ...
      <div className="mb-6 p-4 border border-gray-200 rounded-lg bg-gray-50">
        <label className="block text-sm font-medium text-gray-700 mb-3">🖼️ Company Logo</label>
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <label
              htmlFor="logo-upload"
              className={`inline-block px-4 py-2 border border-blue-600 text-blue-600 rounded-lg cursor-pointer hover:bg-blue-50 ${uploading ? 'opacity-50' : ''}`}
            >
            ...
            <p className="text-xs text-gray-500">Used in customer app and receipts</p>
          </div>
        </div>
      </div>
